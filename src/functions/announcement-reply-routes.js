...
const upload = multer({
    storage,
    limits: { fileSize: 25 * 1024 * 1024, files: 10 }, // same 25MB per file cap
});
    ...
    async (req, res) => {
        try {
            ...
            if (parent) {
                ...
                parentDoc = await AnnouncementReply.findById(parent).select("_id announcement").lean();
            ...
            const doc = await AnnouncementReply.create({
                announcement: id,
                parent: parent || null,
                author: req.user._id,
                contentHtml,
...
replyRoutes.post(
    ...
    async (req, res) => {
        try {
            ...
            res.json({ ok: true, restored: replyId });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
    }
...
replyRoutes.get(
    ...
    async (req, res) => {
        try {
            ...
            const parent = await AnnouncementReply.findById(replyId).select("_id announcement").lean();
